...
void encs_read(void) {
  ...
  for (byte enc = 0; enc < 18; enc++) {
    ...
    if (!encs_lastA[enc] && (busA)) { // If pin A is RISING (was low and is high),
      encs_newIndex = enc;            // This is the one
      busB = digitalRead(BUS_B_PIN);  // Read the status of the bus on pinB
      encs_newAction = busB ? 1 : -1; // Get the direction
    }
    encs_lastA[enc] = busA;           // Save busA value for next read of this pin

    digitalWrite(pin, HIGH);          // Disabled read for this pin
    if (encs_newIndex != -1) return;  // This was the one
  }
}